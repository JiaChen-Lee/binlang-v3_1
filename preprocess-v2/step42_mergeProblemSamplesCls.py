# Created by Jiachen Li at 2020/12/28 23:05
import os
import shutil
...
def foo(root, dst_root):
    for cls_name in os.listdir(root):
        ...
        if not cls_name.endswith("backup") and os.path.isdir(cls_path):
            for sub_cls_name in os.listdir(cls_path):
                ...
                if os.path.isdir(sub_cls_path) and sub_cls_name[-2] == "0":
                    for sub_sub_cls_name in os.listdir(sub_cls_path):
                        sub_sub_cls_path = os.path.join(sub_cls_path, sub_sub_cls_name)

                        for channel_name in os.listdir(sub_sub_cls_path):
                            channel_path = os.path.join(sub_sub_cls_path, channel_name)
                            ...
                            for img_name in os.listdir(channel_path):
                                ...
                                new_img_name = sub_sub_cls_name + "_" + channel_name + '_' + img_name
